...
#import "OLFacebookImageDownloadDelegate.h"

@interface OLFacebookImageDownloader ()

@end

@implementation OLFacebookImageDownloader

+ (instancetype)sharedInstance {
    static dispatch_once_t once;
    static OLFacebookImageDownloader *sharedInstance;
    dispatch_once(&once, ^{
...
- (NSURLSessionDownloadTask *)downloadImageAtURL:(NSURL *)url progress:(void(^)(NSInteger progress, NSInteger total))progressHandler withCompletionHandler:(void(^)(UIImage *image, NSError *error))handler{
    NSURLRequest *request = [[NSURLRequest alloc] initWithURL:url];
    OLFacebookImageDownloadDelegate *delegate = [[OLFacebookImageDownloadDelegate alloc] init];
